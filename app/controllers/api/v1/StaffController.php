...
class API_v1_StaffController extends ApiGuardController {
    ...
    public function all() {
        ...
        $users = $company->users()->where('role', User::ROLE_COMPANY)->paginate(100);
        return $this->response->withPaginator($users, new UserTransformer, 'staff');
    }

    public function show($userId) {
        $company = Company::where('company_id', $this->apiKey->user_id)->first();
        $user = $company->users()->where('role', User::ROLE_COMPANY)->where('user_id', $userId)->first();
        if (!$user) {
